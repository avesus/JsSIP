...
const debug = require('react-native-debug')('JsSIP:RTCSession');
const debugerror = require('react-native-debug')('JsSIP:ERROR:RTCSession');
...
//so that this branch works across r-n-webrtc versions for now
WebRTC.mediaDevices = {
...
module.exports = class RTCSession extends EventEmitter
{
      ...
      .then(() =>
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          ...
          .catch((error) =>
          {
            ...
            debugerror('emit "peerconnection:setremotedescriptionfailed answer" [error:%o]', error);
          ...
          if (this._late_sdp)
          {
            ...
            this._connectionPromiseQueue = this._connectionPromiseQueue
              ...
              .catch((error) =>
              {
                ...
                debugerror('emit "peerconnection: receive request" [error:%o]', error);
                this.emit('peerconnection:setremotedescriptionfailed', error);
              });
          }
          else
  ...
  onTransportError()
  {
    ...
    if (this._status !== C.STATUS_TERMINATED)
    {
      //temporary fix, really we should only terminate the call if the media also fails
      setTimeout(() => {
  ...
  _createLocalDescription(type, constraints)
  {
    ...
    return Promise.resolve()
      ...
      {
        let newData = {}
        Object.assign(newData, desc)
        if (type === 'answer')
        {
          if (connection.iceGatheringState === 'complete')
          {
            newData.sdp = newData._sdp.replace(/a=inactive/g, 'a=recvonly');
            newData.type = 'answer';
          }
          else
          {
            newData.sdp = newData._sdp.replace(/a=recvonly/g, 'a=inactive');
            newData.type = 'pranswer';
          }
        }
        const foo = (desc) => {
          return connection.setLocalDescription(desc)
        }
        return foo(type === 'answer' ? newData:desc)
          .catch((error) =>
          {
            this._rtcReady = true;
    ...
    this._connectionPromiseQueue = this._connectionPromiseQueue
      ...
      .then(() =>
      {
        ...
        return this._connection.setRemoteDescription(offer)
          ...
          {
            request.reply(488);
            debugerror('emit "peerconnection:setremotedescriptionfailed in dialog sdp" [error:%o]', error);
    ...
    switch (true)
    {
      ...
      case /^1[0-9]{2}$/.test(response.status_code):
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          ...
          .catch((error) =>
          {
            debugerror('emit "peerconnection:setremotedescriptionfailed 1xx invite response" [error:%o]', error);
          ...
          .then(() =>
          {
            this._connection.setRemoteDescription(answer)
              ...
              .catch((error) =>
              {
                ...
                debugerror('emit "peerconnection:setremotedescriptionfailed 2xx invite response" [error:%o]', error);
    ...
    function onSucceeded(response)
    {
      ...
      this._connectionPromiseQueue = this._connectionPromiseQueue
        ...
        .catch((error) =>
        {
          ...
          debugerror('emit "peerconnection:setremotedescriptionfailed sendReinvite onsucceded" [error:%o]', error);
      ...
      if (sdpOffer)
      {
        ...
        this._connectionPromiseQueue = this._connectionPromiseQueue
          ...
          .catch((error) =>
          {
            ...
            debugerror('emit "peerconnection:setremotedescriptionfailed sendupdate onsucceeded" [error:%o]', error);
